...
typeof(c)
```
In finance, the weighted average is an important calculation. It allows you to calculate your portfolio return over a time period. Let's consider an example to see how we can use vectors to compute weighted average.
...
Let's use vectors to store __returns__ and __weights__
```{r}  
returns <-  c(5, 7)
weights <- c(0.4, 0.6)
...
In finance, you can compute present value using following equation 
...
```{r}
cash <- data.frame(company = rep(c("A","B"),each = 2), cash_flow = c(1000, 4000, 200, 500),year = c(1,3, 1,2))
cash
