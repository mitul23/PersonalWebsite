...
In addition to forcasts, we will also use other tidyverse packages, including ggplot2, dplyr, stringr, and tidyr along with two sample datasets- **_fivethirtyeight_** flight dataset and **_Kaggle’s State of Data Science and ML Survey_**. 
...
Now we can find the number of _levels_ in each column with the following code
```{r, collapse=TRUE}
...
ggplot(mc_responses_reordered,  aes(fct_infreq(WorkInternalVsExternalTools))) + 
    geom_bar() + 
    coord_flip()
